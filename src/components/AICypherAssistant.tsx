...
export const AICypherAssistant: React.FC<AICypherAssistantProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/70 backdrop-blur-sm animate-fadeIn">
      <div className="sleek-card border-l border-blue-500/30 w-full max-w-md h-full flex flex-col justify-between shadow-2xl relative text-slate-100">
        
        {/* Header */}
        <div className="p-4 bg-slate-900/90 dark:bg-slate-900/90 light:bg-slate-100 border-b border-slate-800/80 dark:border-slate-800/80 light:border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl sleek-glow-btn flex items-center justify-center text-white shadow-md">
              <Zap className="w-5 h-5 text-white" />
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <h3 className="font-bold text-slate-900 dark:text-white text-base">CypherAI Chatbox</h3>
                <span className="flex h-2 w-2 relative">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
                </span>
              </div>
              <p className="text-[10px] text-blue-400 font-mono tracking-tight flex items-center gap-1">
        ...
        <div className="px-4 py-2 bg-blue-950/40 border-b border-blue-500/20 text-[11px] font-mono text-blue-300 dark:text-blue-300 light:text-blue-700 flex items-center justify-between">
          <span className="flex items-center gap-1.5">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
            <span>Endpoint: /webhook/Cypherdevs</span>
          </span>
          <span className="text-[10px] bg-blue-900/60 px-2 py-0.5 rounded text-blue-200">n8n Cloud</span>
        </div>
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-4">
          {messages.map((msg, i) => (
            ...
            >
              {msg.sender === 'ai' && (
                <div className="w-7 h-7 rounded-lg bg-blue-950/80 dark:bg-blue-950/80 light:bg-blue-100 border border-blue-500/40 flex items-center justify-center text-blue-400 shrink-0 mt-1">
              ...
              <div className="max-w-[85%] space-y-1">
                <div
                  className={`p-3.5 rounded-2xl text-xs leading-relaxed ${
                    msg.sender === 'user'
                      ? 'sleek-glow-btn text-white font-medium rounded-tr-none'
                      : 'bg-slate-900/80 dark:bg-slate-900/80 light:bg-slate-100 border border-slate-800/80 dark:border-slate-800/80 light:border-slate-200 text-slate-200 dark:text-slate-200 light:text-slate-700 rounded-tl-none shadow-md'
                  }`}
                >
                  {msg.text.split('\n').map((line, lineIdx) => {
                    let cleanLine = line;
          ...
          {loading && (
            <div className="flex items-center gap-2 text-xs text-blue-400 font-mono animate-pulse bg-blue-950/30 p-2.5 rounded-xl border border-blue-500/20 max-w-[80%]">
        ...
        <div className="p-4 bg-slate-900/90 dark:bg-slate-900/90 light:bg-slate-100 border-t border-slate-800/80 dark:border-slate-800/80 light:border-slate-200 space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {quickPrompts.map((promptText, i) => (
              <button
                ...
                className="text-[11px] text-blue-400 dark:text-blue-400 light:text-blue-600 bg-slate-900/60 dark:bg-slate-900/60 light:bg-white hover:bg-slate-800/80 border border-slate-800/80 dark:border-slate-800/80 light:border-slate-200 hover:border-blue-500/40 px-2.5 py-1 rounded-lg text-left truncate max-w-full cursor-pointer transition-colors"
              >
                {promptText}
              </button>
            ))}
          ...
          >
            <input
              ...
              className="flex-1 px-3.5 py-2.5 rounded-xl bg-slate-900/80 dark:bg-slate-900/80 light:bg-white border border-slate-800 dark:border-slate-800 light:border-slate-300 text-xs text-slate-900 dark:text-white placeholder:text-slate-500 outline-none focus:border-blue-500"
            />
            <button
              type="submit"
              disabled={loading}
              className="p-2.5 rounded-xl sleek-glow-btn text-white font-bold transition-colors disabled:opacity-50 cursor-pointer"
            >
          ...
          <button
            ...
            className="w-full py-2 rounded-lg text-[11px] font-bold bg-slate-900/60 dark:bg-slate-900/60 light:bg-white hover:bg-slate-800/80 text-blue-400 dark:text-blue-400 light:text-blue-600 border border-slate-800/80 dark:border-slate-800/80 light:border-slate-200 text-center cursor-pointer"
          >

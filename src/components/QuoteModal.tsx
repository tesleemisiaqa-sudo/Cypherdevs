...
export const QuoteModal: React.FC<QuoteModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fadeIn">
      <div className="sleek-card rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 shadow-2xl relative text-slate-100">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">{t('requestQuote')}</h3>
        ...
        {submitted ? (
          <div className="py-12 text-center space-y-4">
            ...
            <h4 className="text-xl font-bold text-slate-900 dark:text-white">{lang === 'ar' ? 'تم استلام طلب التقييم بنجاح!' : 'Proposal Request Received!'}</h4>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 dark:text-slate-300 light:text-slate-700 mb-1">{t('companyName')}</label>
                <input
                  type="text"
                  value={companyName}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-semibold text-slate-300 dark:text-slate-300 light:text-slate-700 mb-1">Service Category</label>
                <select
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 dark:text-slate-300 light:text-slate-700 mb-1">{t('budgetRange')}</label>
                <select
                  value={projectBudget}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 dark:text-slate-300 light:text-slate-700 mb-1">{t('timeline')}</label>
                <select
                  value={timeline}
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-300 dark:text-slate-300 light:text-slate-700 mb-1">{t('messageDetails')} *</label>
              <textarea
                required
                rows={4}
